...
from hypersp.utils.helpers import process_evaluation_batch_greedy, process_evaluation_batch_beamsearch, process_evaluation_batch_beamsearch_lm, process_evaluation_epoch, add_ctc_labels, print_dict, model_multi_gpu, __ctc_decoder_predictions_tensor, __beamsearch_decoder_predictions_tensor
from hypersp.models import JasperEncoderDecoder
from hypersp.modules import AudioPreprocessing, GreedyCTCDecoder, CTCDecoder, BeamSearchDecoderWithLM
...
def parse_args():
    ...
    parser.add_argument("--lm", default=None, help="n-gram lm model path")
    # FIXME Unused, but passed by Triton helper scripts
    parser.add_argument("--pyt_fp16", action='store_true',
                        help='use half precision')
    parser.add_argument("--preds_output_folder", default=None, type=str, help="The cache file for storing the outputs of the model")

    return parser.parse_args()


def calc_wer(data_layer, audio_processor,
             encoderdecoder, greedy_decoder,ctc_decoder,
             labels, args, device):
    ...
    with torch.no_grad():
        ...
        _global_var_dict_greedy = {
            'predictions': [],
            'transcripts': [],
            'logits': [],
        }
        
        _global_var_dict_beamsearch = {
            'predictions': [],
            'transcripts': [],
            'logits': [],
        }
        _global_var_dict_beamsearch_lm = {
            'predictions': [],
            'transcripts': [],
            'logits': [],
        }
        print('lm path:', os.path.join(args.dataset_dir, args.lm))
        alpha = 2
        beta = 3.0
        scorer = Scorer(alpha=alpha, beta=beta, model_path=os.path.join(args.dataset_dir, args.lm), vocabulary=labels[:-1])
        ctc_beam_size = 128
        ...
        if args.preds_output_folder:
             preds_output_file = os.path.join(args.preds_output_folder, f"preds_out_width{ctc_beam_size}_alpha{alpha}_beta{beta}.tsv")
        ...
        for it, data in enumerate(tqdm(data_layer.data_iterator)):
            ...
            if not device.type == 'cpu' and args.amp:
            ...
            #beamsearch_decoder
            prob_tensor, pred_tensor = ctc_decoder(log_probs=t_log_probs_e, beam_size=5)
            
            values_dict_greedy = dict(
            ...
            m = nn.Softmax(dim=-1)
            values_dict_beamsearch_lm = dict(
                    predictions=[ctc_beam_search_decoder_batch(m(t_log_probs_e.squeeze()), labels[:-1], beam_size=128, num_processes=4, cutoff_prob=1.0, cutoff_top_n=40, ext_scoring_func=scorer)],
                transcript=[t_transcript_e],
                transcript_length=[t_transcript_len_e],
                # output=[t_log_probs_e] #TODO FIX MEMORY LEAK ISSUE
            )
            # values_dict will contain results from all workers
            # print('lm predictions :', values_dict_beamsearch_lm['predictions'][0][0])
            if preds_output_file:
                for beam in values_dict_beamsearch_lm['predictions'][0]:
                    for score, pred_text in beam:
            ...
            values_dict_beamsearch = dict(
                predictions=[dict({'prob_cpu_tensor': prob_cpu_tensor, 'pred_cpu_tensor' : pred_cpu_tensor})],
                transcript=[t_transcript_e],
                transcript_length=[t_transcript_len_e],
                # output=[t_log_probs_e] #TODO FIX MEMORY LEAK ISSUE
            )

            process_evaluation_batch_greedy(
                values_dict_greedy, _global_var_dict_greedy, labels=labels)
            
            process_evaluation_batch_beamsearch(
                values_dict_beamsearch, _global_var_dict_beamsearch, labels=labels)

            process_evaluation_batch_beamsearch_lm(
                values_dict_beamsearch_lm, _global_var_dict_beamsearch_lm, labels=labels)

            if args.steps is not None and it + 1 >= args.steps:
                break
        
        # final aggregation (over minibatches) and logging of results
        greedy_cer, _ = process_evaluation_epoch(_global_var_dict_greedy, use_cer=True)
        greedy_wer, _ = process_evaluation_epoch(_global_var_dict_greedy)
        beamsearch_cer, _ = process_evaluation_epoch(_global_var_dict_beamsearch, use_cer=True)
        beamsearch_wer, _ = process_evaluation_epoch(_global_var_dict_beamsearch)

        beamsearch_lm_cer, _ = process_evaluation_epoch(_global_var_dict_beamsearch_lm, use_cer=True)
        beamsearch_lm_wer, _ = process_evaluation_epoch(_global_var_dict_beamsearch_lm)
        print(f'greedy_cer : {greedy_cer*100}%')
        print(f'greedy_wer : {greedy_wer*100}%')
        print(f'beamsearch_cer : {beamsearch_cer*100}%')
        print(f'beamsearch_wer : {beamsearch_wer*100}%') 
        print(f'beamsearch_lm_cer : {beamsearch_lm_cer*100}%')
        print(f'beamsearch_lm_wer : {beamsearch_lm_wer*100}%')
        return beamsearch_cer, beamsearch_wer, _global_var_dict_beamsearch
 #       return cer, wer, _global_var_dict_greedy

def run_once(audio_processor, encoderdecoder, greedy_decoder, ctc_decoder, audio, audio_len, labels, device, type='greedy'):
...
def eval(
    ...
    with torch.no_grad():
        if args.wav:
            ...
            run_once(audio_processor, encoderdecoder, greedy_decoder,ctc_decoder,
                     audio, audio_len, labels, device)
            return
        cer, wer, _global_var_dict = calc_wer(
            data_layer, audio_processor, encoderdecoder, greedy_decoder, ctc_decoder, labels, args, device)
